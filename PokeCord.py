...
class PokeCord(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.author == self.bot.user or message.content[1:].startswith("spawn") or message.channel.type != discord.ChannelType.text:  # or message.channel == CHANNEL_IDs:
            return
    ...
    @commands.command(name="inventory", aliases=["backpack"], help="Displays pokemon and items")
    ...
    @commands.command(name="trade", help="Trade a pokemon between two users")
        ...
        if member.id in self.trainer_list and ctx.author.id in self.trainer_list:
            ...
            if not trainer_give.hasPokemon(give_pokemon):
                ...
                def check(reaction, user):
                    ...
                    return user == member and reaction.message.id == sent_message.id and (reaction.emoji == "\u2716" or reaction.emoji == "\u2714")
    ...
    async def _spawn(self, channel=None, message=None):
        ...
        poke = pokemon.from_message(message)
    ...
    async def convert_gif_bw(self, poke):
        ...
        # Save the gif
        all_frames[0].save(txtfile_name, save_all=True, append_images=all_frames[1:], optimize=False, duration=frames.info["duration"], loop=0, disposal=1, transparency=255, background=0)
        # Upload to Imgur
        try:
