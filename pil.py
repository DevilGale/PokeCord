...
class PilPokemon:
    ...
    def getGIF(self, pokemon_id) -> Image.Image:
        ...
        self.response = requests.get(
            f"https://play.pokemonshowdown.com/sprites/xyani/{name.lower()}.gif"
        )
    ...
    def _method_dispose(self, i, frames, previous_frame):
        ...
        new_frame.alpha_composite(
            current_frame, dest=frames.dispose_extent[0:2], source=frames.dispose_extent
        )
    ...
    def showCollageGIF(self, images: list, num_frame_columns: int = 8):
        ...
        compilation = Image.new(
            "RGBA", size=(width * num_frame_columns, height * num_frame_rows)
        )
        for i in range(len(images)):
            ...
            dest = (
                width * int(i % num_frame_columns),
                height * int(i / num_frame_columns),
            )
    ...
    def saveGIF(self, images: list, filename: str = "test.gif"):
        images[0].save(
            ...
            optimize=False,
            duration=self.original_image.info["duration"],
            loop=0,
            disposal=1,
            transparency=255,
            background=0,
...
def simpleCollage(
    frames: Image.Image,
    num_images_width: int = 5,
    num_images_height: int = 10,
    fit: bool = False,
):
    ...
    print(
        f"Frames in image ({num_images_width}x{num_images_height}): {frames.n_frames} - {frames.filename if frames.filename else 'N/A'}"
    )
    compilation = Image.new(
        "RGBA", size=(width * num_images_width, height * num_images_height)
    )
...
def reformCollage(
    frames, num_images_width: int = 5, num_images_height: int = 10, fit: bool = False
):
    """
    ...
    if fit:
        num_images_height = math.ceil(frames.n_frames / num_images_width)
    print(
        f"Frames in image ({num_images_width}x{num_images_height}): {frames.n_frames} - {frames.filename}"
    )
    frames.seek(0)
    backgroundColor = frames.info["background"]
    compilation = Image.new(
        "RGBA",
        size=(width * num_images_width, height * num_images_height),
        color=(255, 255, 0),
    )  # , color=(255,0,0))
    fnt = ImageFont.load_default().font
...
def method_gather_colors(
    current_frame: Image, hitBlackLine=False, colorsToCheck=[transparent]
):
